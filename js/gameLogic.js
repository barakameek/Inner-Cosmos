// --- START OF MODIFIED gameLogic.js (with RoleFocus Integration) ---

// js/gameLogic.js - Application Logic (Updated for 7 Elements)
...
console.log("gameLogic.js loading... (with RoleFocus integration)");

// --- Temporary State ---
let currentlyDisplayedConceptId = null;
let currentReflectionContext = null;
let reflectionTargetConceptId = null;
let currentReflectionCategory = null; // Could be Element Name or Concept Name etc.
...
export function countUndiscoveredByRarity(elementKey) { // No changes needed, logic is sound
    ...
    concepts.forEach(concept => {
        if (!discoveredIds.has(concept.id)) {
            ...
            // Now check for RF score as well
            const score = concept.elementScores?.[elementKey] || 0;
            // Consider relevance if primary OR score is high enough
            const isRelevant = isPrimary || score >= 5.0;
            if (isRelevant) {
...
export function gainAttunementForAction(actionType, elementKey = null, amount = 0.5) {
    ...
    if (elementKey && State.getAttunement().hasOwnProperty(elementKey)) {
        targetKeys.push(elementKey);
         ...
         if (currentReflectionContext === 'Standard' && currentReflectionCategory) {
              // currentReflectionCategory is now the Element Name (e.g., "RoleFocus")
              ...
              keyFromContext = scene?.element || null; // Scenes might not have RF yet
              ...
              keyFromContext = cEntry ? cEntry[1].concept.primaryElement : null; // Uses concept's primary element
         }
    ...
    targetKeys.forEach(key => {
        if (State.updateAttunement(key, baseAmount)) {
            changed = true;
            // Trigger milestone checks (includes RF check if key is 'RF' or 'All')
    ...
    if (changed) {
        ...
        if (document.getElementById('personaScreen')?.classList.contains('current')) {
           UI.displayElementAttunement(); // UI function updated for 7 elements
       }
    }
}
...
// --- Questionnaire Logic (Updated for 7 Elements) ---
...
// --- Starter Hand (Euclidean distance now uses 7 dimensions) ---
...
// --- Core Actions ---
export function displayPersonaScreenLogic() { calculateTapestryNarrative(true); checkSynergyTensionStatus(); UI.displayPersonaScreen(); } // UI updated for 7 elements
export function displayStudyScreenLogic() { UI.displayStudyScreenContent(); } // UI updated for 7 elements

// --- Research Actions ---
export function handleResearchClick({ currentTarget, isFree = false }) { /* No change needed */
    const button = currentTarget; const elementKey = button.dataset.elementKey; const cost = parseFloat(button.dataset.cost); if (!elementKey || isNaN(cost)) { console.error("Invalid research button data."); return; } if (button.classList.contains('disabled')) { console.log("Research button disabled."); return; } if (isFree && State.getInitialFreeResearchRemaining() > 0) { if (State.useInitialFreeResearch()) { console.log(`Used initial free research on ${elementKey}.`); conductResearch(elementKey); UI.displayStudyScreenContent(); } else { console.warn("Attempted free research, but none remaining."); UI.showTemporaryMessage("No free research attempts left.", 3000); } } else { if (spendInsight(cost, `Research: ${elementKeyToFullName[elementKey]}`)) { console.log(`Spent ${cost} Insight on ${elementKey}.`); conductResearch(elementKey); updateMilestoneProgress('conductResearch', 1); checkAndUpdateRituals('conductResearch'); UI.displayStudyScreenContent(); } }
}
export function handleFreeResearchClick() { /* No change needed, iterates over 7 elements */
...
export function handleConfirmReflection(nudgeAllowed) { // Now handles RF attunement gain context
    ...
    // Dissonance handling (updated nudge loop for 7 elements)
    if (currentReflectionContext === 'Dissonance') { milestoneAct = 'completeReflectionDissonance'; attuneAmt = 0.5; if (nudgeAllowed && reflectionTargetConceptId) { console.log("Processing nudge..."); const concept = concepts.find(c => c.id === reflectionTargetConceptId); const scores = State.getScores(); let nudged = false; if (concept?.elementScores) { const newScores = { ...scores }; for (const key in scores) { // Will now loop through 7 keys including RF
            if (scores.hasOwnProperty(key) && concept.elementScores.hasOwnProperty(key)) { const uScore = scores[key]; const cScore = concept.elementScores[key]; const diff = cScore - uScore; if (Math.abs(diff) > 1.5) { const nudgeVal = Math.sign(diff) * Config.SCORE_NUDGE_AMOUNT; newScores[key] = Math.max(0, Math.min(10, uScore + nudgeVal)); if (newScores[key] !== uScore) nudged = true; } } } if (nudged) { State.updateScores(newScores); console.log("Nudged Scores (7 Elements):", State.getScores()); UI.displayPersonaScreen(); UI.showTemporaryMessage("Core understanding shifted.", 3500); gainAttunementForAction('scoreNudge', 'All', 0.5); updateMilestoneProgress('scoreNudgeApplied', 1); } } else { console.warn(`Cannot apply nudge, concept data missing for ID ${reflectionTargetConceptId}`); } } if (reflectionTargetConceptId) { addConceptToGrimoireInternal(reflectionTargetConceptId, 'dissonance'); } }
    gainInsight(rewardAmt, `Reflection (${currentReflectionContext || 'Unknown'})`);
    // Determine Attunement Key (Now includes RF possibility)
    if (currentReflectionContext === 'Standard' && currentReflectionCategory) {
        attuneKey = elementNameToKey[currentReflectionCategory]; // currentReflectionCategory holds Element Name like "RoleFocus"
    } else if (currentReflectionContext === 'Guided') {
        attuneKey = null;
    } else if (currentReflectionContext === 'RareConcept') {
        const cEntry = Array.from(State.getDiscoveredConcepts().entries()).find(([id, data]) => data.concept.uniquePromptId === currentPromptId);
        attuneKey = cEntry ? cEntry[1].concept.primaryElement : null; // Includes possibility of RF as primary
    } else if (currentReflectionContext === 'SceneMeditation') {
        const scene = sceneBlueprints.find(s => s.reflectionPromptId === currentPromptId);
        attuneKey = scene?.element || null; // Scene elements likely don't include RF yet
    } else {
        attuneKey = null;
    }
    if (attuneKey) gainAttunementForAction('completeReflection', attuneKey, attuneAmt);
    else gainAttunementForAction('completeReflectionGeneric', 'All', 0.2); // Generic gain across all 7
...
function attemptExperimentInternal(experimentId) { // Added check for RF score
    const exp = alchemicalExperiments.find(e => e.id === experimentId); if (!exp || State.getRepositoryItems().experiments.has(experimentId)) { console.warn(`Exp ${experimentId} not found/completed.`); return; } const attune = State.getAttunement(); const focused = State.getFocusedConcepts(); const insight = State.getInsight(); const scores = State.getScores();
    // Check Attunement & NEW RoleFocus Score requirement
    if (attune[exp.requiredElement] < exp.requiredAttunement) { UI.showTemporaryMessage("Attunement too low.", 3000); return; }
    if (exp.requiredRoleFocusScore && (scores.RF || 0) < exp.requiredRoleFocusScore) { UI.showTemporaryMessage(`Requires RoleFocus Score: ${exp.requiredRoleFocusScore}`, 3000); return; }
    if (exp.requiredRoleFocusScoreBelow && (scores.RF || 0) >= exp.requiredRoleFocusScoreBelow) { UI.showTemporaryMessage(`Requires RoleFocus Score Below: ${exp.requiredRoleFocusScoreBelow}`, 3000); return; }
    // Check Focus (unchanged logic)
...
// --- Synergy/Tension Logic (Updated for 7 Elements) ---
...
// --- Rituals & Milestones Logic (Updated for 7 Elements) ---
export function checkAndUpdateRituals(action, details = {}) { /* No change needed, checks RF automatically if defined in data */ let ritualCompletedThisCheck = false; const currentState = State.getState(); const completedToday = currentState.completedRituals.daily || {}; const focused = currentState.focusedConcepts; let currentRitualPool = [...dailyRituals]; if (focusRituals) { focusRituals.forEach(ritual => { if (!ritual.requiredFocusIds || ritual.requiredFocusIds.length === 0) return; const reqIds = new Set(ritual.requiredFocusIds); let allFoc = true; for (const id of reqIds) { if (!focused.has(id)) { allFoc = false; break; } } // Check RF score requirements if they exist
...
// --- Persona Calculation Logic Helpers (Updated for 7 Elements) ---
...
export function calculateTapestryNarrative(forceRecalculate = false) { // Updated for 7 Elements
    ...
    if (sortedElements.length > 0) {
        ...
        // TODO: Enhance elementInteractionThemes or use procedural logic for 7-element themes
        ...
        // Determine Essence Title (Adjust thresholds maybe?)
...
// --- Elemental Dilemma Logic (Updated for 7 Elements) ---
...
export function handleConfirmDilemma() { // Updated nudge logic for 7 elements
    ...
    if (nudgeAllowed) {
        ...
        if (nudged) { State.updateScores(newScores); console.log("Nudged Scores after Dilemma (7 Elements):", State.getScores()); UI.displayPersonaScreen(); UI.showTemporaryMessage("Dilemma choice influenced core understanding.", 3500); gainAttunementForAction('dilemmaNudge', 'All'); updateMilestoneProgress('scoreNudgeApplied', 1); }
