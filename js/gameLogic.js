...
export function handleConfirmReflection(nudgeAllowed) {
    ...
    // --- Check if research popup might be open ---
    const researchPopupIsOpen = researchResultsPopup && !researchResultsPopup.classList.contains('hidden');
    let hideResearchAfter = !researchPopupIsOpen; // Default to hide unless we keep it open
    ...
    if (currentReflectionContext === 'Dissonance') {
        ...
        if (reflectionTargetConceptId) {
             // Add concept AFTER confirming dissonance
             addConceptToGrimoireInternal(reflectionTargetConceptId, 'dissonanceConfirm');
             // If dissonance was triggered from research popup, update that item
             if (researchPopupIsOpen) {
                 UI.handleResearchPopupAction(reflectionTargetConceptId, 'kept_after_dissonance');
                 hideResearchAfter = false; // Keep research popup open
             }
        }
    }
    ...
    // --- Updated Hide Logic ---
    // Hide popups, but potentially exclude research popup if dissonance was just resolved
    UI.hidePopups(!hideResearchAfter);
    UI.showTemporaryMessage("Reflection complete! Insight gained.", 3000);
    // Only fully clear state if research popup is also hidden
    if (hideResearchAfter) {
        clearPopupState();
    } else { // Partially clear, keep research context maybe? Or rely on popup state?
        // For now, let hidePopups handle conditional clearPopupState based on visibility
        console.log("Reflection confirmed, keeping research popup potentially open.");
    }
}
